...
function PLUGIN:Call( ply, args )
	if ( ply:EV_HasPrivilege( "Jail" ) ) then
		...
		--Remove previous jail
		for _,v in pairs(ents.FindByName("Jail")) do
			v:Remove()
		end
		
		--Spawn Jail
		local angle = Vector(ply:GetAngles().p, ply:GetAngles().y, 180)
		local jail =  ents.Create( "prop_physics" )
			jail:SetName("Jail")
			jail:SetModel("models/props_junk/TrashDumpster02.mdl")
			jail:SetMaterial("models/props_combine/health_charger_glass")
			jail:SetPos(evolve.jailPos+Vector(0,0,60))
			jail:SetAngles(angle)
			jail:Spawn()
