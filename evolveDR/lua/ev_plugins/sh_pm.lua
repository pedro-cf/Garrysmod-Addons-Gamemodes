	...
	if ( ply:EV_HasPrivilege( "Private messages" ) ) then
		...
		if ( #players < 2 or !players[1] ) then			
			if ( #players > 0 ) then
				...
				if ( #msg > 0 ) then
					evolve:Notify( ply, evolve.colors.white, "To ", Color(0,0,255), players[1]:Nick(), evolve.colors.white, ": " .. msg )
					evolve:Notify( players[1], evolve.colors.white, "From ", Color(0,0,255), ply:Nick(), evolve.colors.white, ": " .. msg )
				else
